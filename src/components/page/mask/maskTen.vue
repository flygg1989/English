<template>
    <div class="mask maskone">
         <el-dialog :title="formdata.title" :visible.sync="editVisible" :close-on-click-modal="false" width="864px">
            <el-form ref="formdata" :model="formdata">
                ...
                <div v-for="(item,index) in formdata.log" :key="index">
                    ...
                    <div v-if="item.op_status == 11">
                        ...
                        <el-input type="textarea" :readonly="isReadOnlyOne" autosize v-model="item.reason" placeholder=""></el-input>
                        <div class="modify" v-if="buttononestate == 1">
            ...
            <span slot="footer" class="dialog-footer">
                <el-button type="primary" @click="adoptsubmit()">提交修改</el-button>
            </span>
        </el-dialog>
...
export default {
    ...
    created(){
        Bus.$on('sendID',(data)=>{
            console.log(data)
            if(data.plat_status == 11 || data.plat_status == 12){
                ...
                api.request({
                    ...
                    if(res.status == 200){
                        this.formdata={
                            ...
                            log:res.data.data.common.log,   //不予处理原因   
    ...
    methods: {
        ...
        adoptsubmit(){
            ...
            //再提交一个不予处理的修改
            api.request({
                url: 'suggest/log/save',
                method: "post",
                data:{
                    id: this.id,
                    logId:this.formdata.log[0].id,
                    content:this.formdata.log[0].reason,
                }
            })
            .then(
            res => {
