  ...
  <div class="tabs-content" v-loading="isLoad">
    <el-table
      ...
      <el-table-column
        ...
        width="168">
        <template slot-scope="scope">
          <el-button @click="handleClick(scope.row)" plain size="mini" type="primary">回复</el-button>
          <el-button @click="handleClick(scope.row)" plain size="mini" type="primary">申请退回</el-button>
        </template>
      </el-table-column>
    </el-table>
    ...
    sortChange(value){
      ...
      if(value.prop == "created_at"){{
        if(value.order == 'ascending'){
          this.listData.order = 'created_at asc'
        }else if(value.order == 'descending'){
          this.listData.order = 'created_at desc'
        }
      }}
    ...
    getList:function(data){
      ...
      api.request({
        ...
        if(res.status == 200){
          ...
          Bus.$emit('tableDataChange', this.listData,'2-1-1')//------发送表格参数至operation
