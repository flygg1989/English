  ...
  <div class="tabs-content" v-loading="isLoad">
    <el-table
      ...
      <el-table-column
        ...
        width="72">
        <template slot-scope="scope">
          <div>{{scope.row.suggest.address[0]}}</div>
        </template>
      </el-table-column>
      <el-table-column
        label="市"
        width="72">
        <template slot-scope="scope">
          <div :style="scope.row.suggest.address[1]?'':'text-align:center;'">{{scope.row.suggest.address[1]?scope.row.suggest.address[1]:'/'}}</div>
        </template>
      </el-table-column>
      <el-table-column
        label="区"
        width="72">
        <template slot-scope="scope">
          <div :style="scope.row.suggest.address[2]?'':'text-align:center;'">{{scope.row.suggest.address[2]?scope.row.suggest.address[2]:'/'}}</div>
        </template>
      </el-table-column>
      <el-table-column
        label="部门"
        width="88">
        <template slot-scope="scope">
          <div>{{scope.row.suggest.dept_name}}</div>
        </template>
      </el-table-column>
      <el-table-column
        prop="updated_at"
        ...
        width="144">
        <template slot-scope="scope">
          <div>{{scope.row.suggest.updated_at.substring(0,scope.row.suggest.updated_at.length-3)}}</div>
        </template>
      </el-table-column>
      <el-table-column
        prop="verifyName"
        ...
        width="104">
        <template slot-scope="scope">
          <span :class="scope.row.verifyName == '审核不通过'?'font-red':''">{{ scope.row.verifyName }}</span>
        </template>
      </el-table-column>
      <el-table-column
        prop="reason"
        ...
        <template slot-scope="scope">
          <el-tooltip class="table-tooltip" visible-arrow="" effect="light" v-if="scope.row.verifyName == '审核不通过'">
            ...
            <el-button type="text" class="hover-btn">驳回原因</el-button>
          </el-tooltip>
        </template>
      </el-table-column>
    </el-table>
    ...
    getList:function(data){
      ...
      api.request({
        ...
        if(res.status == 200){
          ...
          Bus.$emit('tableDataChange', this.listData,'1-4-2')//------发送表格参数至operation
