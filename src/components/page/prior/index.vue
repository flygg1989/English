    ...
    <div class="notabs-content" v-loading="isLoad">
      ...
      <el-table
        ...
        <el-table-column
          ...
          width="144">
          <template slot-scope="scope">
            <div>{{scope.row.top_time.substring(0,scope.row.top_time.length-3)}}</div>
          </template>
        </el-table-column>
        <el-table-column
          label="回复结果"
        ...
        <el-table-column
          ...
          width="200">
          <template slot-scope="scope">
            <el-button @click="upOrDown(scope.row,1)" plain size="mini" type="primary" :disabled="listData.order == 'top_sort asc,top_time asc'">
              <i class="el-icon-caret-top"></i>
            </el-button>
            <el-button @click="upOrDown(scope.row,2)" plain size="mini" type="primary" :disabled="listData.order == 'top_sort asc,top_time asc'">
              <i class="el-icon-caret-bottom"></i>
            </el-button>
            <el-button @click="handleCommand(scope.row)" plain size="mini" type="primary">取消置顶</el-button>
          </template>
        </el-table-column>
      </el-table>
...
export default {
  ...
  data(){
    return {
      ...
      statusList:[{value:'3',text:'追问待审'},{value:'4',text:'追问已审'},{value:'5',text:'回复待审'},{value:'6',text:'回复已审'},{value:'10',text:'回复驳回'},{value:'12',text:'追问不予处理'},{value:'14',text:'已评价'},{value:'15',text:'已办结'},{value:'16',text:'追问撤回'}],
      tableList:[],//------------表格列表
      amount:0,//----------------表格数据总数
      typeList:[],//-------------类型筛选列表
      deptList:[],//-------------部门.......
      ...
      listData:{//---------------表格请求参数
        ...
        dept_id:'',
        area:'',
        order:'top_sort desc,top_time desc',
        f:'',
        t:'',
        title:'',
        page:1,
...
//---      筛选函数      ----//
    ...
    sortChange(value){
      ...
      //按置顶时间排序
      if(value.prop == "top_time"){{
        if(value.order == 'ascending'){
          this.listData.order = 'top_sort asc,top_time asc'
        }else if(value.order == 'descending'){
          this.listData.order = 'top_sort desc,top_time desc'
        }
      }}
      this.getList(this.listData)
